...
function AddNewView({ onCancel, onSuccess }) {
  ...
  const validate = () => {
    ...
    if (!['publish', 'pending', 'draft'].includes(post['post_status'])) {
      newErrors['post_status'] = __('Invalid status selected', 'cforge');
    }
    ...
    if (post['post_type'] === 'page' && post['post_parent'] && post['post_parent'] !== '0') {
      ...
      if (!validParent) {
        newErrors['post_parent'] = __('Invalid parent page selected', 'cforge');
      }
    }
    if (tab === 'auto') {
      const num = Number(post['post_number']);
      if (!num || num < 1) {
        newErrors['post_number'] = __('Number of Pages/Posts must be at least 1', 'cforge');
      }
    } else if (tab === 'manual') {
      if (!allowedPostTypes.includes(post['post_type'])) {
        newErrors['post_type'] = __('Invalid type selected', 'cforge');
      }
      if (!post['post_title'] || !post['post_title'].trim()) {
        newErrors['post_title'] = __('Please enter at least one title', 'cforge');
      }
    }
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitting(false);
      setNotice({
        message: tab === 'auto' ? __('Pages/Posts generated successfully!', 'cforge') : __('Pages/Posts added successfully!', 'cforge'),
        status: 'success',
      });
      setTimeout(() => {
        setNotice(null);
      ...
      <div className="cforge-flex cforge-gap-4">
        <div className="cforge-w-1/2">
          ...
          >
            ...
            <div className="cforge-flex-1">
              <p className="cforge-block cforge-text-sm cforge-font-medium cforge-text-gray-900">{__('Auto Generate', 'cforge')}</p>
              <p className="cforge-text-sm cforge-text-gray-500">{__('Auto generate post/page name and contents', 'cforge')}</p>
            </div>
            {tab === 'auto' && (
        ...
        <div className="cforge-w-1/2">
          ...
          >
            ...
            />
            <div className="cforge-flex-1">
              <p className="cforge-block cforge-text-sm cforge-font-medium cforge-text-gray-900">{__('Manual', 'cforge')}</p>
              <p className="cforge-text-sm cforge-text-gray-500">{__('Manually input post/page name and contents', 'cforge')}</p>
            </div>
            {tab === 'manual' && (
              <svg className="cforge-h-5 cforge-w-5 cforge-text-primary" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" data-slot="icon">
                <path fillRule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.857-9.809a.75.75 0 0 0-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 1 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.137-.089l4-5.5Z" clipRule="evenodd" />
        ...
        <form
            ...
            {tab === 'auto' ? (
              <>
                <div className="cforge-mb-4">
                  <label className="cforge-block cforge-mb-1 cforge-font-medium">{__('Number of Pages/Posts', 'cforge')}</label>
                  <input
                    type="number"
                    min="1"
                ...
                <div className="cforge-mb-4">
                  <label className="cforge-block cforge-mb-1 cforge-font-medium">{__('Type', 'cforge')}</label>
                  <select
                    className={`cforge-input ${errorClass('post_type')}`}
                    value={post['post_type']}
                    onChange={e => setPost({ ...post, post_type: e.target.value })}
                  >
                    <option value="post">{__('Post', 'cforge')}</option>
                    <option value="page">{__('Page', 'cforge')}</option>
                  </select>
                  {errors['post_type'] && <p className="cforge-text-red-500 cforge-text-sm">{errors['post_type']}</p>}
                </div>
                <div className="cforge-mb-4">
                  <label className="cforge-block cforge-mb-1 cforge-font-medium">{__('Pages/Posts Status', 'cforge')}</label>
                  <select
                    className={`cforge-input ${errorClass('post_status')}`}
                    value={post['post_status']}
                    onChange={e => setPost({ ...post, post_status: e.target.value })}
                  >
                    <option value="publish">{__('Publish', 'cforge')}</option>
                    <option value="pending">{__('Pending', 'cforge')}</option>
                    <option value="draft">{__('Draft', 'cforge')}</option>
                  </select>
                  {errors['post_status'] && <p className="cforge-text-red-500 cforge-text-sm">{errors['post_status']}</p>}
                </div>
                <div className="cforge-mb-4">
                  <label className="cforge-block cforge-mb-1 cforge-font-medium">{__('Comment Status', 'cforge')}</label>
                  <select
                  ...
                  >
                    <option value="closed">{__('Closed', 'cforge')}</option>
                    <option value="open">{__('Open', 'cforge')}</option>
                  </select>
                  {errors['comment_status'] && <p className="cforge-text-red-500 cforge-text-sm">{errors['comment_status']}</p>}
                </div>
                {post.post_type === 'page' && (
                  <div className="cforge-mb-4">
                    <label className="cforge-block cforge-mb-1 cforge-font-medium">{__('Parent Page', 'cforge')}</label>
                    <select
                      className={`cforge-input ${errorClass('post_parent')}`}
                      value={post['post_parent']}
                      onChange={e => setPost({ ...post, post_parent: e.target.value })}
                    >
                      <option value="0">{__('No Parent', 'cforge')}</option>
                      {pages.map((page) => (
              ...
              <>
                ...
                <div className="cforge-mb-4">
                  <label className="cforge-block cforge-mb-1 cforge-font-medium">{__('Titles (comma separated)', 'cforge')}</label>
                  <input
                    type="text"
                  ...
                  <p className="cforge-text-sm cforge-text-gray-500">{__('eg. Page1, Page2, page3, PAGE4, PAge5', 'cforge')}</p>
                </div>
                <div className="cforge-mb-4">
                  <label className="cforge-block cforge-mb-1 cforge-font-medium">{__('Page/Post content', 'cforge')}</label>
                  <textarea
                  ...
                  <p className="cforge-text-sm cforge-text-gray-500">{__('eg. This is the content of the page/post', 'cforge')}</p>
                </div>
                <div className="cforge-mb-4">
                  <label className="cforge-block cforge-mb-1 cforge-font-medium">{__('Pages/Posts Status', 'cforge')}</label>
                  <select
                  ...
                  >
                    ...
                    <option value="pending">{__('Pending', 'cforge')}</option>
                    <option value="draft">{__('Draft', 'cforge')}</option>
                    <option value="private">{__('Private', 'cforge')}</option>
                  </select>
                  {errors['post_status'] && <p className="cforge-text-red-500 cforge-text-sm">{errors['post_status']}</p>}
                </div>
                <div className="cforge-mb-4">
          ...
          <div className="cforge-flex cforge-justify-end cforge-mt-6 cforge-gap-2">
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? __('Generating...', 'cforge') : __('Generate', 'cforge')}
            </button>
          </div>
        </form>
      </div>
    ...
    <div className="cforge-bg-white cforge-p-8 cforge-min-h-screen">
      ...
      {view === 'list' ? (
        <ListView
          endpoint="posts"
          columns={[
            { key: 'title', label: __('Title', 'cforge') },
            { key: 'author', label: __('Author', 'cforge') },
            { key: 'type', label: __('Type', 'cforge') },
          ...
          renderRow={(item) => (
            <>
              <td
                ...
                title={__('Edit this post/page', 'cforge')}
              >
                {item.title}
              </td>
