    ...
    return (
        <div className="cforge-flex cforge-items-center cforge-justify-between cforge-mb-6">
            ...
            <div className="cforge-flex cforge-items-center cforge-gap-2">
                {!telemetryEnabled && (
                    <button
                        onClick={handleTelemetryOptIn}
                        disabled={isOptingIn}
                        className="cforge-btn cforge-btn-secondary"
                        title={__('Help improve Content Forge by sharing anonymous usage data', 'content-forge')}
                    >
                        {isOptingIn ? __('Enabling...', 'content-forge') : __('📊 Enable Telemetry', 'content-forge')}
                    </button>
                )}
                {onAddNew && (
                    <button
