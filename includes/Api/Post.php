...
class Post extends CForge_REST_Controller {
    ...
    /**
     * Upper bound on terms assigned per taxonomy, per generated post.
     *
     * @since 1.7.0
     *
     * @var int
    ...
    public function handle_bulk_create( $request ) {
        ...
        if ( ! in_array( $post_status, [ 'publish', 'pending', 'draft', 'private', 'future' ], true ) ) {
        ...
        // Scheduled posts need an explicit publish date that is still in the future.
        $post_date     = isset( $params['post_date'] ) ? sanitize_text_field( $params['post_date'] ) : '';
        $post_date_gmt = '';
        if ( 'future' === $post_status ) {
            $scheduled_ts = $post_date ? strtotime( $post_date ) : false;
            if ( ! $scheduled_ts ) {
                return new \WP_REST_Response(
                    [ 'message' => __( 'A valid publish date is required for scheduled posts.', 'content-forge' ) ],
                    400
                );
            }
            // The submitted value is site wall-clock time; store it as-is and derive GMT from it.
            $post_date     = gmdate( 'Y-m-d H:i:s', $scheduled_ts );
            $post_date_gmt = get_gmt_from_date( $post_date );
            if ( strtotime( $post_date_gmt . ' UTC' ) <= time() ) {
                return new \WP_REST_Response(
                    [ 'message' => __( 'Scheduled date must be in the future.', 'content-forge' ) ],
                    400
                );
            }
        }
        ...
        // Resolve taxonomy term assignment options (empty = assign nothing).
        $taxonomy_options = $this->sanitize_taxonomy_options(
            isset( $params['taxonomy_options'] ) ? $params['taxonomy_options'] : [],
            $post_type
        );
        ...
        if ( $use_ai ) {
            // AI posts are generated by a background queue, which has no publish-date handling.
            if ( 'future' === $post_status ) {
                return new \WP_REST_Response(
                    [ 'message' => __( 'Scheduled status is not available for AI generation. Choose draft, pending, private, or publish.', 'content-forge' ) ],
                    400
                );
            }
            ...
            if ( ! empty( $taxonomy_options ) ) {
                $ai_args['taxonomy_options'] = $taxonomy_options;
            }
        ...
        // (date range or schedule, and author pool). The generator strips the
        // range/author keys before insert; post_date is passed through as-is.
        $date_range_args = [];
        if ( 'future' === $post_status ) {
            // An explicit schedule wins over a randomized date range.
            $date_range_args['post_date']     = $post_date;
            $date_range_args['post_date_gmt'] = $post_date_gmt;
        ...
        if ( ! empty( $taxonomy_options ) ) {
            $date_range_args['taxonomy_options'] = $taxonomy_options;
        }
        $created   = [];
    ...
    /**
     * Sanitize the taxonomy assignment options submitted with a bulk request.
     *
     * Drops taxonomies that are not assignable for the post type, term IDs that
     * do not exist in their taxonomy, and clamps the per-post term count so a
     * crafted request cannot force an unbounded number of term writes.
     *
     * @since 1.7.0
     *
     * @param mixed  $options   Raw taxonomy options from the request body.
     * @param string $post_type Target post type slug.
     *
     * @return array<string, array> Sanitized options keyed by taxonomy slug.
     */
    private function sanitize_taxonomy_options( $options, $post_type ) {
        if ( ! is_array( $options ) || empty( $options ) ) {
            return [];
        }

        $assignable = wp_list_pluck( \cforge_get_assignable_taxonomies( $post_type ), 'name' );
        $clean      = [];

        foreach ( $options as $taxonomy => $option ) {
            $taxonomy = sanitize_key( $taxonomy );

            if ( ! is_array( $option ) || ! in_array( $taxonomy, $assignable, true ) ) {
                continue;
            }

            $mode = isset( $option['mode'] ) ? sanitize_key( $option['mode'] ) : 'none';
            if ( ! in_array( $mode, [ 'specific', 'random' ], true ) ) {
                continue;
            }

            $terms = [];
            if ( 'specific' === $mode ) {
                $requested = isset( $option['terms'] ) && is_array( $option['terms'] ) ? $option['terms'] : [];
                foreach ( array_map( 'absint', $requested ) as $term_id ) {
                    if ( $term_id && term_exists( $term_id, $taxonomy ) ) {
                        $terms[] = $term_id;
                    }
                }
                // A specific selection with nothing valid left means nothing to assign.
                if ( empty( $terms ) ) {
                    continue;
                }
            }

            $min = isset( $option['min'] ) ? absint( $option['min'] ) : 1;
            $max = isset( $option['max'] ) ? absint( $option['max'] ) : $min;
            $min = min( max( $min, 1 ), self::MAX_TERMS_PER_TAXONOMY );
            $max = min( max( $max, 1 ), self::MAX_TERMS_PER_TAXONOMY );
            if ( $min > $max ) {
            ...
            if ( 'specific' === $mode ) {
                $clean[ $taxonomy ]['terms'] = array_values( array_unique( $terms ) );
            }
        }
    ...
    private function format_post_items( $post_ids, $post_types = null ) {
        ...
        foreach ( $posts as $post ) {
            ...
            // 'raw' keeps the ampersands unencoded; the helper returns null when the
            // current user cannot edit, which is what hides the row action.
            ...
            $formatted_items[] = [
                ...
                'permalink' => esc_url_raw( (string) get_permalink( $post->ID ) ),
            ];
        }
        return $formatted_items;
    }
