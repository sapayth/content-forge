...
class Comment extends CForge_REST_Controller
{
	...
	public function handle_bulk_create($request)
	{
		...
		if ($comment_number < 1) {
			return new \WP_REST_Response(['message' => __('Number of comments must be at least 1.', 'content-forge')], 400);
		}
		...
		if ($comment_post_ID > 0) {
			...
			if (!$post || $post->post_status !== 'publish' || $post->comment_status !== 'open') {
				return new \WP_REST_Response(['message' => __('Invalid post ID or post does not allow comments.', 'content-forge')], 400);
			}
		}
		...
		if (empty($created)) {
			return new \WP_REST_Response(['message' => __('Failed to generate comments. No suitable posts found or other error occurred.', 'content-forge')], 500);
		}
		...
		return new \WP_REST_Response([
			...
			'message' => sprintf(
				__(
					...
					'Successfully deleted %d comments.',
					'content-forge'
				),
				$deleted
			),
	...
	private function get_tracked_comments_count()
	{
		...
		if ($wpdb->last_error) {
			return new \WP_Error('database_error', __('Database error occurred while counting comments.', 'content-forge'));
		}
	...
	private function get_tracked_comments_ids($pagination_params)
	{
		...
		if ($wpdb->last_error) {
			return new \WP_Error('database_error', __('Database error occurred while fetching comment IDs.', 'content-forge'));
		}
